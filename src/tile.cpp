...
void Tile::onAddTileItem(Item* item)
{
	...
	const SpectatorVec& list = g_game.getSpectators(m_pos);
	...
	for (it = list.begin(); it != list.end(); ++it) {
		if ((tmpPlayer = (*it)->getPlayer())) {
			tmpPlayer->sendAddTileItem(this, m_pos, item);
		}
	}

	// event methods
	for (it = list.begin(); it != list.end(); ++it) {
		(*it)->onAddTileItem(this, m_pos, item);
	}
}
...
void Tile::onUpdateTileItem(Item* oldItem, const ItemType& oldType, Item* newItem, const ItemType& newType)
{
	...
	for (it = list.begin(); it != list.end(); ++it) {
		if ((tmpPlayer = (*it)->getPlayer())) {
			tmpPlayer->sendUpdateTileItem(this, m_pos, oldItem, newItem);
		}
	}

	// event methods
	for (it = list.begin(); it != list.end(); ++it) {
		(*it)->onUpdateTileItem(this, m_pos, oldItem, oldType, newItem, newType);
	}
}
...
void Tile::onRemoveTileItem(const SpectatorVec& list, std::vector<int32_t>& oldStackposVector, Item* item)
{
	...
	// event methods
	for (it = list.begin(); it != list.end(); ++it) {
		(*it)->onRemoveTileItem(this, m_pos, iType, item);
	}
}
...
void Tile::onUpdateTile()
{
	...
	for (it = list.begin(); it != list.end(); ++it) {
		if ((tmpPlayer = (*it)->getPlayer())) {
			tmpPlayer->sendUpdateTile(this, m_pos);
		}
	}

	// event methods
	for (it = list.begin(); it != list.end(); ++it) {
		(*it)->onUpdateTile(this, m_pos);
	}
}
...
void Tile::moveCreature(Creature* actor, Creature* creature, Cylinder* toCylinder, bool forceTeleport /* = false*/)
{
	...
	g_game.getSpectators(list, m_pos, true, false);
	Position newPos = newTile->getPosition();
	g_game.getSpectators(list, newPos, true, false);

	bool teleport = false;
	if (forceTeleport || !newTile->ground || !Position::areInRange<1, 1, 0>(m_pos, newPos)) {
		teleport = true;
	}
	...
	// event method
	for (it = list.begin(); it != list.end(); ++it) {
		(*it)->onCreatureMove(creature, newTile, newPos, this, m_pos, teleport);
	}
	...
	uint32_t flags, Creature*) const
{
	...
	if (const Creature* creature = thing->getCreature()) {
			...
			if (hasBitSet(FLAG_IGNOREBLOCKITEM, flags)) // if the FLAG_IGNOREBLOCKITEM bit isn't set we dont have to iterate every single item
			{
				...
				if (items) {
					for (Item* tileItem : *items) {
						const ItemType& iType = Item::items[tileItem->getID()];
						if (tileItem->isBlocking(creature) && (!iType.movable || (tileItem->isLoadedFromMap() && (tileItem->getUniqueId() || (tileItem->getActionId() && tileItem->getContainer()))))) {
							return RET_NOTPOSSIBLE;
						}
					}
				}
		...
		if (ground) {
			if (ground->isBlocking(nullptr)) {
				const ItemType& iType = Item::items[ground->getID()];
				if (!iType.allowPickupable || item->isBlocking(nullptr)) {
		...
		if (items) {
			if (isHangable) {
				...
				for (ItemVector::const_iterator it = items->begin(); it != items->end(); ++it) {
					...
					if (!(*it)->isBlocking(nullptr) || iType.allowPickupable) {
						continue;
					}
...
void Tile::__addThing(Creature* actor, int32_t, Thing* thing)
{
	...
	if (item->isGroundTile()) {
		if (ground) {
			...
			postRemoveNotification(actor, oldGround, nullptr, oldGroundIndex, true);
			onUpdateTile();
		} else {
			ground = item;
			++m_thingCount;
		...
		if (item->isSplash()) {
			...
			if (items) {
				for (ItemVector::iterator it = items->getBeginTopItem(); it != items->getEndTopItem(); ++it) {
					...
					postRemoveNotification(actor, oldSplash, nullptr, oldSplashIndex, true);
					break;
				}
			}
		}
			...
			if (items) {
				...
				for (ItemVector::iterator it = items->getBeginDownItem(); it != items->getEndDownItem(); ++it) {
					...
					if (oldField->isReplacable()) {
						...
						postRemoveNotification(actor, oldField, nullptr, oldFieldIndex, true);
						break;
					}
...
void Tile::__replaceThing(uint32_t index, Thing* thing)
{
	...
	if (!item) {
#ifdef __DEBUG_MOVESYS__
		std::clog << "[Failure - Tile::__replaceThing] item == nullptr" << std::endl;
#endif
		return /* RET_NOTPOSSIBLE*/;
	}
	...
	if (item->isAlwaysOnTop()) {
		...
		if (it != items->end()) {
			if (item->isStackable() && count != item->getItemCount()) {
				...
				const ItemType& iType = Item::items[item->getID()];
				onUpdateTileItem(item, iType, item, iType);
			} else {
				const SpectatorVec& list = g_game.getSpectators(m_pos);
				std::vector<int32_t> oldStackposVector;
...
{
	SpectatorVec list;
	g_game.getSpectators(list, m_pos, true, true);
	...
	int32_t index, bool isCompleteRemoval, CylinderLink_t /* link = LINK_OWNER*/)
{
	SpectatorVec list;
	g_game.getSpectators(list, m_pos, true, true);
	if (/*isCompleteRemoval && */ m_thingCount > 8) {
		onUpdateTile();
	}
