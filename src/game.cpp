...
namespace
{
	...
	std::pair<MagicEffect_t, Color_t> getCombatDetails(CombatType_t combatType, Creature* target, bool createSplash)
	{
		...
		switch (combatType) {
			case COMBAT_PHYSICALDAMAGE: {
				switch (target->getRace()) {
					...
					case RACE_BLOOD: {
						color = COLOR_RED;
						effect = MAGIC_EFFECT_DRAW_BLOOD;
						if (createSplash) {
					...
					case RACE_ENERGY: {
						color = COLOR_PURPLE;
						effect = MAGIC_EFFECT_PURPLEENERGY;
						break;
					}
			...
			case COMBAT_ENERGYDAMAGE: {
				color = COLOR_PURPLE;
				effect = MAGIC_EFFECT_ENERGY_DAMAGE;
				break;
			}
			case COMBAT_EARTHDAMAGE: {
				color = COLOR_LIGHTGREEN;
				effect = MAGIC_EFFECT_POISON_RINGS;
				break;
			}
			case COMBAT_DROWNDAMAGE: {
				color = COLOR_LIGHTBLUE;
				effect = MAGIC_EFFECT_LOSE_ENERGY;
				break;
			}
			case COMBAT_FIREDAMAGE: {
				color = COLOR_ORANGE;
				effect = MAGIC_EFFECT_HITBY_FIRE;
				break;
			}
			case COMBAT_ICEDAMAGE: {
				color = COLOR_TEAL;
				effect = MAGIC_EFFECT_ICEATTACK;
				break;
			}
			case COMBAT_HOLYDAMAGE: {
				color = COLOR_YELLOW;
				effect = MAGIC_EFFECT_HOLYDAMAGE;
				break;
			}
			case COMBAT_DEATHDAMAGE: {
				color = COLOR_DARKRED;
				effect = MAGIC_EFFECT_SMALLCLOUDS;
				break;
			}
			case COMBAT_LIFEDRAIN: {
				color = COLOR_RED;
				effect = MAGIC_EFFECT_WRAPS_RED;
				break;
			}
		...
		if (splash) {
			g_game.internalAddItem(nullptr, target->getTile(), splash, INDEX_WHEREEVER, FLAG_NOLIMIT);
			...
			for (; it != trash.end(); ++it) {
				...
				while (tile->getItemList() && tit != tile->getItemList()->end()) {
					if ((*tit)->isMovable() && !(*tit)->isLoadedFromMap()
						...
						internalRemoveItem(nullptr, *tit);
						if (tile->getItemList()) {
							tit = tile->getItemList()->begin();
						}
...
void Game::proceduralRefresh(RefreshTiles::iterator* it /* = nullptr*/)
{
	...
	addSchedulerTask(SCHEDULER_MINTICKS, ([this, it]() { proceduralRefresh(it); }));
}
...
void Game::loadNamesFromXml()
{
	...
	std::clog << ">>>> [Loaded monster names into cache]." << std::endl;
}
	...
	const Position& movingCreaturePos, const Position& toPos, const bool delay)
{
	...
	if (!Position::areInRange<1, 1, 0>(movingCreaturePos, player->getPosition()) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar)) {
		...
		if (getPathToEx(player, movingCreaturePos, listDir, 0, 1, true, true)) {
			addDispatcherTask(([this, playerID = player->getID(), listDir = std::move(listDir)]() {
				playerAutoWalk(playerID, listDir);
			}));

			player->setNextWalkActionTask(createSchedulerTask(100, [=]() {
				playerMoveCreature(playerId, movingCreatureId, movingCreaturePos, toPos, false);
			}));
			return true;
		}
	...
	if (player != movingCreature) {
		if (!player->hasFlag(PlayerFlag_IgnoreProtectionZone) && (movingCreature->getZone() == ZONE_PROTECTION || movingCreature->getZone() == ZONE_OPTIONAL) && !toTile->hasFlag(TILESTATE_OPTIONALZONE)
	...
	const uint16_t spriteId, const int16_t fromStackpos, const Position& toPos, const uint8_t count)
{
	...
	if (!player->canDoAction()) {
		player->setNextActionTask(createSchedulerTask(player->getNextActionTime(), [=]() {
			playerMoveItem(playerId, fromPos, spriteId, fromStackpos, toPos, count);
		}));
		return false;
	}
	...
	if (!Position::areInRange<1, 1, 0>(playerPos, mapFromPos) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar)) {
		...
		if (getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)) {
			...
			player->setNextWalkActionTask(createSchedulerTask(std::max<uint32_t>(SCHEDULER_MINTICKS, player->getStepDuration()), [=]() {
				playerMoveItem(playerId, fromPos, spriteId, fromStackpos, toPos, count);
			}));
			return true;
		}
	...
	if (item->isHangable() && toCylinder->getTile()->hasProperty(SUPPORTHANGABLE)) {
		...
		if (!Position::areInRange<1, 1, 0>(playerPos, mapToPos) && !player->hasCustomFlag(PlayerCustomFlag_CanMoveFromFar)) {
			...
			if (map->getPathTo(player, walkPos, listDir)) {
				...
				player->setNextWalkActionTask(createSchedulerTask(std::max<uint32_t>(SCHEDULER_MINTICKS, player->getStepDuration()), [=]() {
					playerMoveItem(playerId, itemPos, spriteId, itemStackpos, toPos, count);
				}));
				return true;
			}
...
ReturnValue Game::internalMoveItem(Creature* actor, Cylinder* fromCylinder, Cylinder* toCylinder,
	int32_t index, Item* item, const uint32_t count, Item** _moveItem, uint32_t flags /*= 0*/, Item* tradeItem /* = nullptr*/)
{
	if (!toCylinder) {
		return RET_NOTPOSSIBLE;
	}
	...
	uint32_t flags, bool test, uint32_t& remainderCount, Item** stackItem)
{
	*stackItem = nullptr;
	...
	if (item->isStackable() && toItem) {
		...
		if (count > 0) {
			if (item->getItemCount() != count) {
				...
				if ((ret = internalAddItem(nullptr, destCylinder, remainderItem, INDEX_WHEREEVER, flags, false)) == RET_NOERROR) {
	...
	toCylinder->__addThing(nullptr, index, item);
...
ReturnValue Game::internalRemoveItem(Creature* actor, Item* item, int32_t count /* = -1*/, bool test /* = false*/, uint32_t flags /* = 0*/)
{
	...
	if (!test) {
		...
		cylinder->postRemoveNotification(actor, item, nullptr, index, item->isRemoved());
		if (item->isRemoved()) {
			freeThing(item);
		}
	}
...
bool Game::addMoney(Cylinder* cylinder, int64_t money, uint32_t flags /*= 0*/, bool canDrop /*= true*/) // changed by matheus2
{
	...
	for (IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it) {
		...
		do {
			...
			if (internalAddItem(nullptr, cylinder, item, INDEX_WHEREEVER, flags, false, remainderCount) != RET_NOERROR) {
				if (!canDrop) // if canDrop is set to false we end here.
				{
					...
					if (internalAddItem(nullptr, cylinder->getTile(), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR) {
						delete item;
					}
				}
			}
...
Item* Game::transformItem(Item* item, const uint16_t newId, const int32_t newCount /* = -1*/)
{
	...
	if ((item->isContainer() && !newType.isContainer()) || (curType.alwaysOnTop != newType.alwaysOnTop)) // se o item for container e o novo não, ai remove o velho e cria um novo
	{
		...
		if (internalAddItem(nullptr, cylinder, newItem, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR) {
			delete newItem;
			return nullptr;
		}
	...
	cylinder->postAddNotification(nullptr, newItem, cylinder, itemIndex);
...
ReturnValue Game::internalTeleport(Thing* thing, const Position& newPos, const bool forceTeleport, const uint32_t flags /* = 0*/, bool fullTeleport /* = true*/)
{
	...
	if (Tile* toTile = map->getTile(newPos)) {
		if (Creature* creature = thing->getCreature()) {
			if (fullTeleport) {
				return internalMoveCreature(nullptr, creature, creature->getParent(), toTile, flags, forceTeleport);
			}

			creature->getTile()->moveCreature(nullptr, creature, toTile, forceTeleport);
			return RET_NOERROR;
		}
...
bool Game::playerOpenPrivateChannel(const uint32_t playerId, const std::string& receiver)
{
	...
	std::string realName = receiver;
	if (IOLoginData::getInstance()->playerExists(realName)) {
		player->sendOpenPrivateChannel(realName);
	} else {
...
bool Game::playerAutoWalk(const uint32_t playerId, const std::vector<Direction>& listDir)
{
	Player* player = getPlayerByID(playerId);
	if (!player || player->isRemoved()) {
		return false;
	...
	if (player->hasCondition(CONDITION_GAMEMASTER, GAMEMASTER_TELEPORT)) {
		...
		for (auto it = listDir.begin(); it != listDir.end(); ++it) {
	...
	const Position& toPos, const int16_t toStackpos, const uint16_t toSpriteId, bool isHotkey)
{
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, walkToPos, listDir, 0, 1, true, true, 10)) {
				...
				player->setNextWalkActionTask(createSchedulerTask(std::max<uint32_t>(SCHEDULER_MINTICKS, player->getStepDuration()), [=]() {
					playerUseItemEx(playerId, itemPos, fromStackpos, fromSpriteId, toPos, toStackpos, toSpriteId, isHotkey);
				}));
				return true;
			}
	...
	if (!player->canDoAction()) {
		player->setNextActionTask(createSchedulerTask(player->getNextActionTime(), [=]() {
			playerUseItemEx(playerId, fromPos, fromStackpos, fromSpriteId, toPos, toStackpos, toSpriteId, isHotkey);
		}));
		return false;
	}
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, pos, listDir, 0, 1, true, true)) {
				...
				player->setNextWalkActionTask(createSchedulerTask(std::max<uint32_t>(SCHEDULER_MINTICKS, player->getStepDuration()), [=]() {
					playerUseItem(playerId, pos, stackpos, index, spriteId, isHotkey);
				}));
				return true;
			}
	...
	if (ret != RET_NOERROR) {
		if (ret == RET_TOOFARAWAY) {
			...
			if (getPathToEx(player, item->getPosition(), listDir, 0, 1, true, true)) {
				...
				player->setNextWalkActionTask(createSchedulerTask(std::max<uint32_t>(SCHEDULER_MINTICKS, player->getStepDuration()), [=]() {
					playerUseBattleWindow(playerId, pos, stackpos, creatureId, spriteId, isHotkey);
				}));
				return true;
			}
	...
	if (tradePartner->getTradeState() == TRADE_ACCEPT) {
		...
		if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
			...
			if (ret1 == RET_NOERROR && ret2 == RET_NOERROR) {
				...
				ret1 = internalMoveItem(nullptr, cylinder1, tradePartner, INDEX_WHEREEVER, tradeItem1, count1, nullptr, FLAG_IGNOREAUTOSTACK);
				if (ret1 == RET_NOERROR) {
					internalMoveItem(nullptr, cylinder2, player, INDEX_WHEREEVER, tradeItem2, count2, nullptr, FLAG_IGNOREAUTOSTACK);
...
bool Game::playerSay(const uint32_t playerId, const uint16_t channelId, const MessageClasses& type, const std::string& receiver, const std::string& text, bool notify /*= true*/)
{
	...
	switch (type) {
		case MSG_SPEAK_SAY:
			return internalCreatureSay(player, MSG_SPEAK_SAY, text, false, nullptr, nullptr, statementId, false, fakeChat);
...
bool Game::playerWhisper(Player* player, const std::string& text, const uint32_t statementId, bool fakeChat /*= false*/)
{
	...
	internalCreatureSay(player, MSG_SPEAK_WHISPER, text, false, &list, nullptr, statementId, false, fakeChat);
	return true;
}
	...
	const std::string& text, const uint32_t statementId, bool notify /*= true*/, bool fakeChat /*= false*/)
{
	...
	if (fakeChat) {
		if (toPlayer->getIP() == player->getIP()) {
			toPlayer->sendCreatureSay(player, type, text, nullptr, statementId);
...
bool Game::getPathTo(const Creature* creature, const Position& destPos,
	std::vector<Direction>& listDir, const int32_t maxSearchDist /*= -1*/)
{
...
bool Game::getPathToEx(const Creature* creature, const Position& targetPos,
	std::vector<Direction>& dirList, const FindPathParams& fpp)
{
	return map->getPathMatching(creature, dirList, FrozenPathingConditionCall(targetPos), fpp);
}

bool Game::getPathToEx(const Creature* creature, const Position& targetPos, std::vector<Direction>& dirList,
...
bool Game::combatBlockHit(CombatDamage& damage, Creature* attacker,
	Creature* target, bool checkDefense,
	bool checkArmor, bool field/* = false*/)
{
	...
	getSpectators(spectators, targetPos, true, true);

	if (Combat::canDoCombat(attacker, target, true) != RET_NOERROR) {
		addMagicEffect(spectators, targetPos, MAGIC_EFFECT_POFF, target->isGhost());
		return true;
	}

	BlockType_t primaryBlockType, secondaryBlockType;
	if (damage.primary.type != COMBAT_NONE) {
		primaryBlockType = target->blockHit(attacker, damage.primary.type, damage.primary.value, checkDefense, checkArmor, !field, field, false);

		if (primaryBlockType == BLOCK_DEFENSE) {
			addMagicEffect(target->getPosition(), MAGIC_EFFECT_BLOCKHIT);
		} else if (primaryBlockType == BLOCK_ARMOR) {
			addMagicEffect(target->getPosition(), MAGIC_EFFECT_BLOCKHIT);
		} else if (primaryBlockType == BLOCK_IMMUNITY) {
			addMagicEffect(target->getPosition(), MAGIC_EFFECT_POFF);
		}
	} else {
	...
	if (damage.secondary.type != COMBAT_NONE) {
		secondaryBlockType = target->blockHit(attacker, damage.secondary.type, damage.secondary.value, false, false, !field, field, true);
		// elemental damages have no block effect
	} else {
...
bool Game::combatChangeHealth(Creature* attacker, Creature* target,
	CombatDamage& damage, MagicEffect_t hitEffect/* = MAGIC_EFFECT_NONE*/,
	Color_t hitColor/* = COLOR_NONE*/, bool force/* = false*/)
{
	// normalize values
	damage.primary.value = otx::util::abs(damage.primary.value);
	damage.secondary.value = otx::util::abs(damage.secondary.value);
	...
	if (damage.primary.type == COMBAT_HEALING) {
		...
		if (target->hasEventRegistered(CREATURE_EVENT_STATSCHANGE)) {
			for (CreatureEvent* it : target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE)) {
				if (!it->executeStatsChange(target, attacker, STATSCHANGE_HEALTHGAIN, damage.primary.type, damage.primary.value)) {
					deny = true;
				}
			}
		}
		...
		const int32_t oldHealth = target->getHealth();
		...
		if (oldHealth != target->getHealth() && otx::config::getBoolean(otx::config::SHOW_HEALTH_CHANGE) && !target->isGhost() && (otx::config::getBoolean(otx::config::SHOW_HEALTH_CHANGE_MONSTER) || !target->getMonster())) {
			...
			getSpectators(spectators, targetPos, true, true);
			if (damage.primary.type != COMBAT_HEALING) {
				addMagicEffect(spectators, targetPos, MAGIC_EFFECT_WRAPS_BLUE);
			}

			const int32_t changedHealth = (target->getHealth() - oldHealth);
			const char* plural = (changedHealth > 1 ? "s." : ".");

			std::ostringstream ss;
			ss << '+' << changedHealth;
			addAnimatedText(spectators, targetPos, COLOR_MAYABLUE, ss.str());

			ss.str("");
			Player* player = (attacker ? attacker->getPlayer() : nullptr);
			if (player) {
				if (attacker != target) {
					ss << "You healed " << target->getNameDescription() << " for " << changedHealth << " hitpoint" << plural;
				} else {
					ss << "You healed yourself for " << changedHealth << " hitpoint" << plural;
				}

				player->sendTextMessage(MSG_HEALED, ss.str());
				ss.str("");
			}
			...
			if (player && attacker != target) {
				if (attacker) {
					ss << ucfirst(attacker->getNameDescription()) << " heals you for " << changedHealth << " hitpoint" << plural;
				} else {
					ss << "You are healed for " << changedHealth << " hitpoint" << plural;
				}

				player->sendTextMessage(MSG_HEALED, ss.str());
			}
		}
	} else if (damage.primary.type != COMBAT_NONE) {
		SpectatorVec spectators;
		getSpectators(spectators, targetPos, true, true);
		if (target->getHealth() < 1 || Combat::canDoCombat(attacker, target, true) != RET_NOERROR) {
			addMagicEffect(spectators, targetPos, MAGIC_EFFECT_POFF);
			return true;
		}
		...
		if (targetPlayer && target->hasCondition(CONDITION_MANASHIELD) && damage.primary.type != COMBAT_UNDEFINEDDAMAGE) {
			int32_t manaDamage = std::min(targetPlayer->getMana(), totalDamage);
			if (manaDamage > 0) {
				if (target->hasEventRegistered(CREATURE_EVENT_STATSCHANGE)) {
					bool deny = false;
					for (CreatureEvent* it : target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE)) {
						if (!it->executeStatsChange(target, attacker, STATSCHANGE_MANALOSS, damage.primary.type, manaDamage)) {
							deny = true;
						}
					}
				...
				targetPlayer->drainMana(attacker, damage.primary.type, manaDamage);
				addMagicEffect(spectators, targetPos, MAGIC_EFFECT_LOSE_ENERGY);
				addAnimatedText(spectators, targetPos, COLOR_BLUE, std::to_string(manaDamage));

				std::ostringstream ss;
				if (attackerPlayer) {
					if (attacker != target) {
						ss << ucfirst(target->getNameDescription()) << " loses " << manaDamage << " mana due to your attack.";
					} else {
						ss << "You lose " << manaDamage << " mana due to your attack.";
					}

					attackerPlayer->sendTextMessage(MSG_HEALED, ss.str());
					ss.str("");
				}

				if (targetPlayer && attacker != target) {
					if (attacker) {
					...
					targetPlayer->sendTextMessage(MSG_HEALED, ss.str());
				}
				...
				totalDamage = damage.primary.value + damage.secondary.value;
				if (totalDamage <= 0) {
					return true;
				}
			}
		...
		if (target->hasEventRegistered(CREATURE_EVENT_STATSCHANGE)) {
			bool deny = false;
			for (CreatureEvent* it : target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE)) {
				if (!it->executeStatsChange(target, attacker, STATSCHANGE_HEALTHLOSS, damage.primary.type, totalDamage)) {
					deny = true;
				}
			}
		...
		if (damage.primary.value != 0) {
			auto&& [effect, color] = getCombatDetails(damage.primary.type, target, true);
			if (hitEffect != MAGIC_EFFECT_NONE) {
				effect = hitEffect;
			}

			if (hitColor != COLOR_NONE) {
				color = hitColor;
			}

			if (effect != MAGIC_EFFECT_NONE) {
				addMagicEffect(spectators, targetPos, effect);
			}

			if (color != COLOR_NONE) {
				addAnimatedText(spectators, targetPos, color, std::to_string(damage.primary.value));
			}
		}

		if (damage.secondary.value != 0) {
			auto&& [effect, color] = getCombatDetails(damage.secondary.type, target, false);
			if (effect != MAGIC_EFFECT_NONE) {
				addMagicEffect(spectators, targetPos, effect);
			}

			if (color != COLOR_NONE) {
				addAnimatedText(spectators, targetPos, color, std::to_string(damage.secondary.value));
			}
		}

		std::ostringstream ss;
		const char plural = (totalDamage > 1 ? 's' : '\0');
		...
		if (player) {
			...
			player->sendTextMessage(MSG_DAMAGE_DEALT, ss.str());
			ss.str("");
		}
		...
		if (player && attacker != target) {
			...
			player->sendTextMessage(MSG_DAMAGE_RECEIVED, ss.str());
		}

		target->drainHealth(attacker, damage.primary.type, damage.primary.value);
		if (damage.secondary.value != 0) {
			target->drainHealth(attacker, damage.secondary.type, damage.secondary.value);
		}
...
bool Game::combatChangeMana(Creature* attacker, Creature* target, CombatDamage& damage)
{
	...
	if (manaChange > 0) {
		...
		const int32_t oldMana = target->getMana();
		...
		if (oldMana != target->getMana() && otx::config::getBoolean(otx::config::SHOW_MANA_CHANGE) && !target->isGhost() && (otx::config::getBoolean(otx::config::SHOW_MANA_CHANGE_MONSTER) || !target->getMonster())) {
			...
			manaChange = (targetPlayer->getMana() - oldMana);
			const char* plural = (manaChange > 1 ? "s." : ".");

			std::ostringstream ss;
			ss << '+' << manaChange;
			addAnimatedText(spectators, targetPos, COLOR_DARKPURPLE, ss.str());
		...
		if (manaLoss > 0) {
			...
			if (target->hasEventRegistered(CREATURE_EVENT_STATSCHANGE)) {
				for (CreatureEvent* it : target->getCreatureEvents(CREATURE_EVENT_STATSCHANGE)) {
					if (!it->executeStatsChange(target, attacker, STATSCHANGE_MANALOSS, damage.primary.type, manaLoss)) {
						deny = true;
					}
				}
			}
			...
			addAnimatedText(targetPos, COLOR_BLUE, std::to_string(manaLoss));
			...
			target->drainMana(attacker, damage.primary.type, manaLoss);
		}
	}
	return true;
}
	...
	std::string comment, std::string statement, const uint32_t statementId, bool ipBanishment)
{
	...
	if (target->isVirtual()) {
		...
		addSchedulerTask(1000, ([this, playerID = target->getID()]() { kickPlayer(playerID, false); }));
	}
...
void Game::checkHighscores()
{
	...
	addSchedulerTask(tmp, [this]() { checkHighscores(); });
}
...
void Game::prepareGlobalSave(uint8_t minutes)
{
	...
	switch (minutes) {
		...
		default:
			if (minutes > 5) {
				addSchedulerTask((minutes - 5) * 1000, [this]() { prepareGlobalSave(5); });
			}
			break;
	}
}
...
void Game::globalSave()
{
	...
	addSchedulerTask(((24 * 60 * 60) - (5 * 60)) * 1000, [this]() { prepareGlobalSave(5); });

	// open server
	addDispatcherTask([this]() { setGameState(GAMESTATE_NORMAL); });
}

void Game::shutdown()
{
	std::clog << "Preparing to shutdown the server... ";
